...
inline bool loadPlaceOffset(double & offsetX, double & offsetY, double & offsetZ, std::string & sourcePath)
{
  try
  {
    sourcePath =
      ament_index_cpp::get_package_share_directory("je_arm_pcb_inspection_sm") +
      "/config/move_group_client/cartesian_offsets/place_offset.yaml";
